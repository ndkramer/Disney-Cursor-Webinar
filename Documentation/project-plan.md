...
- [x] **Step 9 (optional) — Second test with `transcripts/ship-ops-meeting.md`** — same flow as Step 8; status line read "Added 10 cards to the board. Slack: 1 Priority 1 alert sent."; board added 7 To Do, 2 In Progress (`I'm working on it right now with the Raymarine vendor.` → Marcus, `Entertainment — I'm currently rehearsing the new Frozen show segment.` → Toby), and 1 Done (`We finished the Castaway Cay snorkel-tour vendor renegotiation — that's done.`); Captain's "the bridge weather console is showing a critical lag on radar…" P1 alert posted to `#disney-cursor-webinar` (Slack ts `1776718319.270209`). Note: the radar P1 came through with `Assigned to: Unassigned` because the regex extractor doesn't yet attribute speaker context — acceptable for the demo.
- [x] **Post-Session 3 fix — directory case alignment** — on-disk and git-tracked dir is `transcripts/` (lowercase, because macOS HFS+ folded the `Transcripts/` we originally created). Updated `server.js` (`TRANSCRIPTS_DIR`, list-endpoint `path` field, comment) to use `transcripts/` so the app runs correctly on case-sensitive filesystems too.
